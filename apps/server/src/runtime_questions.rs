...
async fn answer_allowed(
    ...
    let authorized:bool=sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM requirement r JOIN execution_revision v ON v.requirement_id=r.id AND v.revision=r.revision CROSS JOIN repository p WHERE r.id=$1 AND r.revision=$2 AND r.state IN ('Running','Failed') AND p.id=COALESCE((v.document->>'repository_id')::bigint,1) AND NOT (p.document->>'revoked')::boolean AND (v.document->>'repository_version')::bigint>p.revoked_through_version)")
