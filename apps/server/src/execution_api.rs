...
async fn delivery(State(pool): State<PgPool>, Path(id): Path<i64>) -> Result<Json<Value>> {
    let deliveries: Vec<Value> = sqlx::query_scalar("WITH records AS (SELECT jsonb_build_object('action_key',d.action_key,'validation_id',d.validation_id,'requirement_id',d.requirement_id,'revision',d.revision,'repository_id',d.repository_id,'repository',d.repository,'branch',d.branch,'head_sha',d.head_sha,'pr_number',d.pr_number,'consumer',d.consumer,'merged',d.released,'actions',(SELECT jsonb_agg(to_jsonb(a)) FROM delivery_action a WHERE a.action_key=d.action_key),'attempts',(SELECT jsonb_agg(to_jsonb(t)) FROM delivery_attempt t WHERE t.action_key=d.action_key)) AS fact,d.* FROM delivery d WHERE requirement_id=$1) SELECT CASE WHEN mode='local_git' THEN fact - 'merged' - 'pr_number' || jsonb_build_object('mode',mode,'repository_id',internal_repository_id,'target',local_binding->'target','delivery_version',CASE WHEN EXISTS(SELECT 1 FROM delivery_observation o WHERE o.action_key=records.action_key AND o.kind='local_git' AND o.fact->>'observation'='delivered' AND o.fact->>'candidate'=records.head_sha) THEN head_sha END,'acceptance',local_acceptance,'storage_blocked',local_storage_blocked,'released',released) ELSE fact END FROM records ORDER BY revision")
