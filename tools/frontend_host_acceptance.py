#!/usr/bin/env python3
"""Local, frontend-only signed Core rehearsal. Not a production trust provisioner.
...
def main():
    ...
    assert (root / '.local-capture-snapshot').read_text() == 'frontend-only rehearsal\n'
    ...
    request_path = config / 'frontend-request.json'
    write(request_path, {})
    # Resolve declaration-only files from pinned AST source, never from a zero
    # counter denominator. They remain inventoried and collected without a numeric
    # coverage requirement for code that TypeScript erases.
    classifier = "const fs=require('fs'),p=require(process.argv[1]); const q=JSON.parse(fs.readFileSync(0,'utf8')); console.log(JSON.stringify(q.map(s=>p.inventory(s.path,fs.readFileSync(s.absolute,'utf8')).declarationOnly)));"
    file_subjects = [s for s in measurement['parameters']['subjects'] if s['kind'] == 'file/v1']
    declaration_flags = json.loads(subprocess.check_output([str(node), '-e', classifier, str(plugin / 'measure.cjs')], text=True, input=json.dumps([{'path': s['path'], 'absolute': str(root / s['path'])} for s in file_subjects])))
    declaration_ids = {s['id'] for s, flag in zip(file_subjects, declaration_flags, strict=True) if flag}
    groups = {'files': [s for s in measurement['parameters']['subjects'] if s['kind'] == 'file/v1' and s['id'] not in declaration_ids],
              'declarations': [s for s in file_subjects if s['id'] in declaration_ids],
              'functions': [s for s in measurement['parameters']['subjects'] if s['kind'] == 'function/v1']}
    groups = {name: subjects for name, subjects in groups.items() if subjects}
    metric_groups = [('files', 'coverage.line'), ('functions', 'coverage.line'),
                     ('functions', 'coverage.function'), ('functions', 'risk.crap')]
...
policy_file = ".harness-gate/frontend-policy.json"
rule = "{name}"
...
'''
    ...
    write(config / 'frontend-policy.json', {'schema': 'harness-policy/v1', 'rules': rules})
    component = {'id': 'frontend', 'path': 'src', 'metadata': {},
                 'targets': [{'id': 'node', 'boundaries': ['production'], 'metadata': {}}],
                 'source_boundaries': [{'id': 'production', 'path': 'src', 'role': 'production', 'metadata': {}}]}
    state = {'schema': 'quality-trusted-state/v1', 'profile': 'full', 'expected': measurement['context'],
             'components': {'frontend': component}, 'subjects': groups,
             'subject_kinds': {name: 'function/v1' if name == 'functions' else 'file/v1' for name in groups}, 'relationship_kinds': {},
             'series': {'frontend': series}, 'artifact_root': 'evidence', 'artifacts': {},
    ...
    def pin():
        state['config_files'] = {'.harness-gate/' + name: digest((config / name).read_bytes())
                                 for name in ('flow.toml', 'quality.toml', 'frontend-policy.json', 'frontend-request.json')}
    ...
    payload = {'schema': 'quality-collector-binding/v1', 'config_files': {p: d for p, d in state['config_files'].items() if p != '.harness-gate/frontend-request.json'}, 'series': state['series'], 'profile': 'full'}
    ...
    write(binding_path, {'schema': 'typescript-project-collector-binding/v1', 'input': inner, 'config_digest': config_digest, 'request': measurement})
    # The executable pin includes every installed runtime file, not just cli.cjs.
    runtime = {str(p): digest(p.read_bytes()) for p in sorted(plugin.rglob('*')) if p.is_file() and 'node_modules' not in p.relative_to(plugin).parts}
    ts_root = plugin.parent.parent / 'typescript'
    if not ts_root.is_dir():
        ts_root = plugin / 'node_modules/typescript'
    assert ts_root.is_dir(), 'missing independently installed TypeScript runtime'
    runtime.update({str(p): digest(p.read_bytes()) for p in sorted(ts_root.rglob('*')) if p.is_file()})
    runtime[str(node)] = digest(node.read_bytes())
    launcher = host / 'collector'
    launcher.write_text('#!/usr/bin/python3\nimport os, sys, hashlib\nfrom pathlib import Path\n'
                        + 'pins = ' + repr(runtime) + '\n'
                        + 'for name, expected in pins.items():\n    assert hashlib.sha256(Path(name).read_bytes()).hexdigest() == expected, "runtime changed"\n'
                        + f'os.execv({str(node)!r}, [{str(node)!r}, {str(plugin / "cli.cjs")!r}, *sys.argv[1:]])\n')
    ...
    write(keys, [{'key_id': 'local-frontend-only', 'public_key': base64.b64encode(public.read_bytes()[12:]).decode()}])
    now = int(time.time() * 1000)
    request = {'protocol_version': 2, 'result_schema_version': '1', 'adapter': measurement['collector'] | {'executable': str(launcher), 'source_digest': digest(launcher.read_bytes()), 'signature': {'algorithm': 'ed25519', 'key_id': 'local-frontend-only', 'value': ''}},
               'invocation_id': measurement['context']['run'], 'step_id': 'frontend', 'timeout_ms': 30000,
    ...
    def sign(value, label):
        ...
        signed['adapter']['signature'] = {'algorithm': 'ed25519', 'key_id': 'local-frontend-only'}
    ...
    result = {'scope': 'local-frontend-only', 'host': str(host), 'collection': 'pass',
              ...
              'declaration_only': [s['path'] for s in file_subjects if s['id'] in declaration_ids]}
