...
def probe():
    ...
    temporary.write_text(json.dumps({'action':'product-preparation-acceptance'}))
    ...
    while time.monotonic()<deadline:
        if result.exists():
            ...
            canonical=json.loads((Path(__file__).parent/'product-preparation-acceptance.json').read_text())
            ...
            return canonical
        time.sleep(.2)
    raise TimeoutError('Host product preparation acceptance did not respond within 240 seconds')
