"""Request the host's fixed reviewed Rust Runtime integration; no external model call."""
...
def probe():
    ...
    temporary.write_text(json.dumps({'action':'runtime-product-acceptance'}))
    ...
    while time.monotonic()<deadline:
        if result.exists():
            ...
            canonical=json.loads((Path(__file__).parent/'runtime-product-acceptance.json').read_text())
