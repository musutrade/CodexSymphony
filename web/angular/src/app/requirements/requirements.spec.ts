...
describe('Requirement browser workflow', () => {
  ...
  async function start() {
    ...
    TestBed.tick();
    http.expectOne('/api/repository').flush(context);
  ...
  it('loads, validates, registers and reports a failed request without losing input', async () => {
    ...
    const req = http.expectOne('/api/repository');
    ...
    const failed = c.save();
    const first = http.expectOne('/api/requirements');
    ...
    const retry = c.save();
    const second = http.expectOne('/api/requirements');
    expect(second.request.body.request_id).toBe(key);
    second.flush(item);
    await Promise.resolve();
    http.expectOne('/api/requirements').flush({ requirements: [item] });
    await retry;
  ...
  it('edits, reviews the observed policy version, withdraws and preserves snapshots', async () => {
    ...
    http.expectOne('/api/requirements/1').flush(item);
    await opened;
    c.addCriterion();
    ...
    const saved = c.save();
    const edit = http.expectOne('/api/requirements/1');
    ...
    const ready = http.expectOne('/api/requirements/1/ready');
    ...
    http.expectOne('/api/requirements/1/withdraw').flush({ ...item, version: 4 });
    await Promise.resolve();
    http.expectOne('/api/requirements').flush({ requirements: [item] });
    await withdrawn;
  ...
  it('renders a load failure then recovers with a fresh read', async () => {
    const fixture = TestBed.createComponent(Requirements);
    TestBed.tick();
    http.expectOne('/api/repository').flush({}, { status: 503, statusText: 'Unavailable' });
    http.expectOne('/api/requirements').flush({ requirements: [] });
