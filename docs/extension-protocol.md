...
外部操作先保存意图再执行；未知结果先 reconcile，确认未发生且冻结策略允许、预算足够时才能重试。任意脚本不承诺恰好一次。停机/取消先监督整组静止并保全，不把 after_run 成功视为验证或交付成功。复用单协调器、现有验证/交付/恢复记录；无插件市场、动态加载、额外事件总线或每概念一张表。生命周期状态通知由 #128 的提交事务 outbox 与独立通知调用接入，见 P14；通知插件没有推进状态的权限。
...
产品登记的 scope_ref 为 `all`、`repositories:<内部 ID 列表>` 或兼容的 `repository:<内部 ID>`，环境 repository_revision 为
...
当前仍是全局单需求执行占用。保留多个工作区和并发通知不意味着引入多需求并行执行；每次恢复与回写继续核对需求/修订/Run/工作区/候选/验证代次。

## 仓库作用域准入

#128 的作用域是核心额外授权，不改变扩展选择、操作能力、资源根或凭据协议。
内部 `repository_id` 是唯一范围身份；`all` 覆盖节点现有及后续仓库，显式列表必须非空、有效且无重复。同仓库所有临时工作区、Run、修复和恢复实例共享范围。不存在长期工作区级作用域。

原生适配器、项目 Hook 及通知的 `plugin_scope` 由部署管理员登记、版本化，调用绑定原版本并复查当前授权。受控扩展的 Registration.scope_ref 保留 `repository:N`，增加 `all`、`repositories:N,M`；原配置摘要即冻结版本，宿主登记漂移会失败关闭，不能静默替换插件。范围缺失、无效或越界阻断所选必需扩展。

通知事件保存仓库快照，核心在建 outbox 之前过滤，在 claim/retry/replay 时重验。v1 通知输入新增 `repository_id`、`scope_version` 两个附加事实字段；现有 v1 插件应忽略未知输入字段，ack 仍严格匹配原 event/attempt。迁移显式登记旧全局范围，新注册不默认 all。扩容不补投历史，缩容不删除原意图/副作用、不重置次数；插件内容筛选不能替代核心授权。配置与迁移细节见 [生命周期恢复](lifecycle-recovery.md#仓库作用域)。
