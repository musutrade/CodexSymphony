...
**11818/12971（91.11%）**，最大 CRAP **10/1**；CRAP 无超限、无不支持/缺失测量。以上为整体覆盖率，不能证明逐函数覆盖率达标。
...
提交 `ed90902557747a8212b6038ad5db85b3f11aa0a9` 的正式 Gate 中，后端/前端测试、编译和 Clippy 均通过，但 12 项逐函数行覆盖与 13 项逐函数 region 覆盖未达到 80%。此前整体覆盖率摘要没有列出这些缺口，不能据此宣称质量门禁通过。

恢复测试覆盖远端查询失败、已关联 PR 消失、取消查询不到 PR、候选清单损坏/变化、App 认证失败、Git 本地执行失败、交付身份缺失、取消不存在需求，以及普通准备失败后的不准入。App push 的失败测试使用临时不存在的本地 Git 目录，Git 在连接远端前退出；HTTP 使用本地夹具，不进行真实 GitHub 写入。

修复后再次执行完整后端测试和原生采集；[逐函数摘要](ci-recovery-summary.json) 检查全部 687 个生产函数的行/region 覆盖率均 ≥80%，CRAP 均 ≤10，未排除源码或放宽阈值。交付/GitHub 聚焦测试共 23 项通过，Clippy `-D warnings` 通过。正式结果以 PR #48 新 head 的双 Gate 为准。

调度阻塞另有一层原因：迁移前两次启动计入累计预算，迁移后的第三次启动完成 PR 后，CI 修复尚未启动就达到 `max_run_attempts=3`。本次人工接手修复，不清零历史预算、不提高全局上限；验收后交回原 PR 的等待/对账阶段。
